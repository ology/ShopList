...
my $sql = 'INSERT INTO item (account_id, name, note, category) VALUES (?, ?, ?, ?)';
my $sth = $dbh->prepare($sql) or die $dbh->errstr;

$sth->execute( 1, 'salmon', 'Fresh!', 'fish' ) or die $dbh->errstr;
$sth->execute( 2, 'Salmon', 'fatty', 'fish' ) or die $dbh->errstr;
